...
info "This script will install bleedingfight/nvimdots to:"
echo "${DEST_DIR}"
...
if [[ "${USE_SSH}" -eq "1" ]]; then
	if check_nvim_version "${REQUIRED_NVIM_VERSION}"; then
		...
		execute "git" "clone" "-b" "0.8" "${CLONE_ATTR[@]}" "git@github.com:bleedingfight/nvimdots.git" "${DEST_DIR}"
	else
...
EOABORT
	...
	if check_nvim_version "${REQUIRED_NVIM_VERSION}"; then
		...
		warn "You have outdated Nvim installed (< ${REQUIRED_NVIM_VERSION})."
		info "Automatically redirecting you to the latest compatible version..."
		execute "git" "clone" "-b" "0.8" "${CLONE_ATTR[@]}" "https://github.com/bleedingfight/nvimdots.git" "${DEST_DIR}"
	else
		warn "You have outdated Nvim installed (< ${REQUIRED_NVIM_VERSION_LEGACY})."
		abort "$(
			cat <<EOABORT
...
- Project Homepage:
    ${tty_underline}https://github.com/bleedingfight/nvimdots${tty_reset}
- Further documentation (including executables you ${tty_bold}must${tty_reset} install for full functionality):
    ${tty_underline}https://github.com/bleedingfight/nvimdots/wiki/Prerequisites${tty_reset}
EOS
